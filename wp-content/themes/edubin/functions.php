...
define('EDUBIN_THEME_VERSION', '6.0.8');
/**
...
function edubin_get_font_url()
{
    ...
    if ('off' !== $bodyFont || 'off' !== $headerFont || 'off' !== $menuFont || 'off' !== $submenuFont) {
        ...
        if ('off' !== $menuFont) {
            $font_families[] = $menuFont['font'] . ':' . $menuFont['regularweight'] . ',' . $menuFont['italicweight'] . ',' . $menuFont['boldweight'];
        }

        if ('off' !== $submenuFont) {
            $font_families[] = $submenuFont['font'] . ':' . $submenuFont['regularweight'] . ',' . $submenuFont['italicweight'] . ',' . $submenuFont['boldweight'];
        }
...
function edubin_customizer_css_styles()
{
    ...
    // Header styles
    $styles .= "h1, h2, h3, h4, h5, h6, .widget .widget-title, .learnpress .lp-single-course .widget-title { font-family: '" . $headerFont['font'] . "'," . $headerFont["category"] . "}";
...
.learnpress.course-item-popup #course-item-content-header .form-button.lp-button-back button { font-family: '" . $bodyFont['font'] . "'," . $bodyFont["category"] . "}";

    // Menu Text styles
    $styles .= ".main-navigation a { font-family: '" . $menuFont['font'] . "'," . $menuFont["category"] . "}";

    // Sub menu Text styles
    $styles .= ".main-navigation ul ul a { font-family: '" . $submenuFont['font'] . "'," . $submenuFont["category"] . "}";
